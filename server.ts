...
// Generic image upload — multipart/form-data with field name 'image'
// Used by logo, favicon, and any other single-image upload fields.
const _imgUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 5 * 1024 * 1024 } });

app.post('/api/admin/upload-image', isAdmin, _imgUpload.single('image'), async (req: any, res) => {
  try {
    const folder = (req.body?.folder as string)?.replace(/[^a-z0-9_-]/gi, '') || 'general';
    const uploadsDir = join(__dirname, 'public', 'uploads', folder);
    ...
    await fs.promises.writeFile(join(uploadsDir, filename), fileBuffer);

...
const WorkOffers = () => {
  ...
  const jobTypes = [
    { key: "all", label: t("workOffersPage.filterAll", { count: jobs.length }) },
    { key: "Full-time", label: t("workOffersPage.filterFullTime") },
    { key: "Part-time", label: t("workOffersPage.filterPartTime") },
    { key: "Contract", label: t("workOffersPage.filterContract") },
    { key: "Seasonal", label: t("workOffersPage.filterSeasonal") },
  ];
            ...
            <nav className={`mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
              <ol className="flex items-center space-x-2 text-sm">
                ...
                <li className="flex items-center">
                  ...
                  <span className="text-white/90">{t("workOffersPage.breadcrumbTalents")}</span>
                </li>
                <li className="flex items-center">
                  <ChevronRight className="w-4 h-4 mx-2 text-white/50" />
                  <span className="text-white font-semibold bg-white/15 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/30 shadow-lg">
                    {t("workOffersPage.breadcrumbWorkOffers")}
                  </span>
                </li>
              </ol>
            </nav>
          ...
          <div className="container mx-auto px-6">
            <div className="flex flex-wrap gap-3">
              {jobTypes.map(jt => (
                <Button
                  key={jt.key}
                  variant={selectedType === jt.key ? "default" : "outline"}
          ...
          <div className="container mx-auto px-6">
            {isLoading ? (
              ...
              <div className="text-center py-16 text-muted-foreground max-w-md mx-auto">
                ...
                <p className="text-xl font-medium">{t("workOffersPage.noOffersTitle")}</p>
                <p className="mt-2">{selectedType !== 'all' ? t("workOffersPage.noOffersFilter") : t("workOffersPage.noOffersDesc")}</p>
              </div>
            ) : (
                    ...
                    <CardContent className="p-6">
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                        <div className="flex-1">
                          ...
                          <h3 className="text-2xl font-bold mb-1 hover:text-primary transition-colors cursor-pointer">{job.title}</h3>
                          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                            <Building2 className="w-4 h-4" /><span className="font-medium">{job.company}</span>
                          </div>
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            {job.location && <div className="flex items-center gap-1"><MapPin className="w-4 h-4" /><span>{job.location}</span></div>}
                            {job.salary && <div className="flex items-center gap-1"><DollarSign className="w-4 h-4" /><span>{job.salary}</span></div>}
                            {job.experience_level && <div className="flex items-center gap-1"><Clock className="w-4 h-4" /><span>{job.experience_level}</span></div>}
                          </div>
                        </div>
                        <Button variant="ghost" size="icon" className="self-start"><Heart className="w-5 h-5" /></Button>
                      </div>
                      ...
                      <div className="grid md:grid-cols-3 gap-4 mb-6 text-sm">
                        {job.responsibilities && job.responsibilities.length > 0 && (
                          <div>
                            <h4 className="font-semibold mb-2 uppercase tracking-wide text-xs">{t("workOffersPage.keyDuties")}</h4>
                            <ul className="space-y-1 text-muted-foreground">
                              {job.responsibilities.slice(0, 3).map((item, idx) => <li key={idx}>• {item}</li>)}
                            </ul>
                          </div>
                        )}
                        {job.requirements && job.requirements.length > 0 && (
                          <div>
                            <h4 className="font-semibold mb-2 uppercase tracking-wide text-xs">{t("workOffersPage.requirements")}</h4>
                            <ul className="space-y-1 text-muted-foreground">
                              {job.requirements.slice(0, 3).map((item, idx) => <li key={idx}>• {item}</li>)}
                            </ul>
                          </div>
                        )}
                        {job.benefits && job.benefits.length > 0 && (
                          <div>
                            <h4 className="font-semibold mb-2 uppercase tracking-wide text-xs">{t("workOffersPage.benefits")}</h4>
                            <ul className="space-y-1 text-muted-foreground">
                              {job.benefits.slice(0, 3).map((item, idx) => <li key={idx}>• {item}</li>)}
                            </ul>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-3">
                        <Button className="flex-1">
                          {t("workOffersPage.applyPosition")}
                          <ArrowRight className="ml-2 w-4 h-4" />
                        </Button>
                        <Button variant="outline">{t("workOffersPage.viewDetails")}</Button>
                      </div>
                    </CardContent>
                  </Card>
                ))}
        ...
        <section className="py-24 bg-gradient-to-br from-primary via-primary to-primary/90 relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="max-w-3xl mx-auto text-center text-white">
              ...
              <h2 className="text-4xl md:text-5xl font-bold mb-6 drop-shadow-lg">{t("workOffersPage.ctaTitle")}</h2>
              <p className="text-lg md:text-xl mb-10 text-white/90 leading-relaxed">{t("workOffersPage.ctaSubtitle")}</p>
